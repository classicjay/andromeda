...
@CrossOrigin(origins = "*")
public class ComputeTaskServiceImpl implements ComputeTaskService,EnvironmentAware {
...
//    private static int step = 10000;
    ...
    public List<Map<String, Object>> run(DataObject dataObject) {
        ...
        keyList =  (List<String>) generateKeyList(dataObject).get("rediskey");
        ...
        if(keyListSize > step){
            ...
            try {
                for(int i = 0; i < threadNum;i++ ){
                    queryResult.addAll((List<String>) completionService.take().get());
                }
            ...
            queryResult = queryDataService.queryRedisData(keyList);
        }
        long endTime = System.currentTimeMillis();
        log.info("查redis总时间"+(endTime - startTime)/1000 );
        if(null == queryResult || queryResult.isEmpty()){
            log.info("redis查询为空，现在查询hbase");
            keyList = (List<Get>) generateKeyList(dataObject).get("hbasekey");
            queryResult = queryDataService.queryData(keyList, dataObject);
        }
        log.info("查询结果集长度："+queryResult.size());
        long startTimeConvert = System.currentTimeMillis();
        //2.查询结果处理，本次循环遍历可以放到计算时做
        List<Map<String, Object>> resultSet = convertResult(dataObject, queryResult);
     ...
     */
    @Override
    public Map<String,Object> generateKeyList(DataObject dataObject){
        String initKey = "DAY";
        ...
        if(recursiveResult.size() == 0 && CommonUtils.isNotBlank(acctDate) && CommonUtils.isNotBlank(kpiCode)
            ...
            if(CommonUtils.isNotBlank(provId) && CommonUtils.isNotBlank(areaNo)) {
                ...
                keyMap.put("hbasekey",keyList);
                keyMap.put("rediskey",rowKeyList);
                return keyMap;
            }
