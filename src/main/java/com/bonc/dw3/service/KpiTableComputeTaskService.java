...
public class KpiTableComputeTaskService extends ComputeTaskServiceImpl implements ComputeTaskService, EnvironmentAware {
    ...
    public List<Map<String, Object>> run(DataObject dataObject) {
        ...
        List<Get> keyList = super.generateKeyList(dataObject);
        ...
        if(keyListSize > step){
            ...
            for(int i=0; i  < threadNum; i++){
                List<Get> subList = keyList.subList(i*step, (i == (threadNum-1) ? ((i+1)*step + other) : (i+1)*step));
                log.info("分给单个线程的keylist长度"+subList.size());
                //taskList.add(new QueryHbaseTask(subList, dataObject));
                completionService.submit(new QueryHbaseTask(subList, dataObject));
            }
            try {
                ...
                for(int i = 0; i < threadNum;i++ ){
                    hbaseResult.addAll((List<String>) completionService.take().get());
                }
        ...
        List<Map<String, Object>> resultSet = super.convertResult(dataObject, hbaseResult);
